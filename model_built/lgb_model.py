# ！/usr/bin/env python
# -*- coding: utf-8 -*-#
# Created by lzy on 2021/6/20 18:29
# @File    :lgb_model.py
...
df["videoplayseconds_log"] = np.log(feed_info["videoplayseconds"] + 1.0)
...
n_day = 5
for stat_cols in tqdm([['userid'],['feedid'],['authorid'],['bgm_song_id'],['bgm_singer_id'],['userid', 'authorid'],['feedid', 'authorid']]):
    f = '_'.join(stat_cols)
...
for f1, f2 in tqdm([['userid', 'feedid'],['userid', 'authorid'],['feedid','authorid']]):
...
## 内存够用的不需要做这一步
feed_embedding = pd.read_csv("E:\\WeChartCompetition\\code\\data\\wechat_algo_data1\\feed_embeddings_pca.csv")
df = pd.merge(df, feed_embedding, how="left", on=["feedid"])
...
cols = [f for f in df.columns if f not in ['date_'] + play_cols + y_list+['videoplayseconds']]
...
test[['userid', 'feedid'] + y_list[:4]].to_csv(
    'sub_%.6f_%.6f_%.6f_%.6f_%.6f.csv' % (uauc_list[0], uauc_list[0], uauc_list[1], uauc_list[2], uauc_list[3]),
    index=False
)
